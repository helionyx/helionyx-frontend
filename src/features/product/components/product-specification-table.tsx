...
const ProductSpecificationTable: React.FC<ProductSpecificationTableProps> = ({ product, models }) => {
	...
	const flattenSpecs = (specs: Specifications): Record<string, SpecGroup> => {
		...
		return {
			Specifications: specs as SpecGroup
		}
	}
	...
	const renderHeader = () => {
		return (
			<TableHeader>
				...
				<TableRow>
					<TableHead className='text-center border bg-muted' colSpan={2} rowSpan={2}>
						Model
					</TableHead>
					{models.map((model) => (
